...
class NgramVerifyInput(SpecInput):
    def __init__(
        ...
        self.draft_lens_cpu = None
        self.num_verify_tokens = None
        self.accept_index_by_req = None
        if draft_lens is not None:
            if hasattr(draft_lens, "detach"):
                self.draft_lens = draft_lens.to(device=self.device, dtype=torch.int64)
                self.draft_lens_cpu = [int(x) for x in self.draft_lens.cpu().tolist()]
            else:
                self.draft_lens_cpu = [int(x) for x in draft_lens]
                self.draft_lens = torch.tensor(
                    self.draft_lens_cpu, dtype=torch.int64, device=self.device
                )
            self.num_verify_tokens = sum(self.draft_lens_cpu)

    @property
    def has_variable_draft_lens(self) -> bool:
    ...
    def prepare_for_verify(self, batch: ScheduleBatch, page_size: int):
        ...
        if self.has_variable_draft_lens and page_size != 1:
            raise RuntimeError(
                "TriePilot variable NGRAM budgets currently require page_size=1."
            )
        ...
        if page_size == 1:
            ...
            if self.has_variable_draft_lens:
                end_offset = batch.seq_lens + self.draft_lens
            else:
    ...
    def generate_attn_arg_prefill(
        ...
        if self.has_variable_draft_lens:
            draft_lens = self.draft_lens.to(
                device=paged_kernel_lens.device,
                dtype=paged_kernel_lens.dtype,
            )
            paged_kernel_lens = paged_kernel_lens + draft_lens
        else:
        ...
        if self.has_variable_draft_lens:
            ...
            self.qo_indptr[1:] = torch.cumsum(
                self.draft_lens.to(dtype=torch.int32), dim=0
            )
        else:
            self.qo_indptr = (
    ...
    def _fill_requests(
        ...
        if self.accept_index_by_req is not None:
            predict_cpu = self.predict.tolist()
            has_finished = False
            flat_accept_indices = []
            accept_lengths = []

            for req, accept_indices in zip(
                batch.reqs, self.accept_index_by_req, strict=True
            ):
                emitted = 0
                ...
                req.spec_accepted_tokens += max(emitted - 1, 0)
                accepted_for_req = accept_indices[:emitted]
                flat_accept_indices.extend(accepted_for_req)
                accept_lengths.append(max(len(accepted_for_req) - 1, 0))

            self.accept_length = torch.tensor(
                accept_lengths, dtype=torch.int32, device=self.device
            )
            self.accept_index = torch.tensor(
                flat_accept_indices, dtype=torch.int32, device=self.device
            )
            accept_index_long = self.accept_index.to(torch.long)
            ...
            self.verified_id = self.predict[accept_index_long]
            return
    ...
    def _free_cache(
        ...
        if self.has_variable_draft_lens:
            ...
            accept_index_long = self.accept_index.to(torch.long)
            evict_mask[accept_index_long] = False
            batch.token_to_kv_pool_allocator.free(batch.out_cache_loc[evict_mask])
            batch.out_cache_loc = batch.out_cache_loc[accept_index_long]
        elif page_size == 1:
    ...
    def _greedy_verify(
        ...
        if self.has_variable_draft_lens:
            self._greedy_verify_variable(batch, logits_output)
            return
    ...
    def _greedy_verify_variable(
        ...
        candidates_cpu = self.draft_token.detach().cpu().tolist()
        next_token_cpu = self.retrive_next_token.detach().cpu().tolist()
        next_sibling_cpu = self.retrive_next_sibling.detach().cpu().tolist()

        predict_cpu = [0] * len(target_predict_cpu)
        accept_index_by_req = []
        offset = 0
        for draft_len in self.draft_lens_cpu:
            last_accepted_idx = offset
            accepted = [offset]
            cur_local_idx = 0

            for _ in range(1, draft_len):
                cur_local_idx = next_token_cpu[offset + cur_local_idx]
                while cur_local_idx != -1:
                    draft_idx = offset + cur_local_idx
                    draft_token_id = candidates_cpu[draft_idx]
                    target_token_id = target_predict_cpu[last_accepted_idx]

                    if draft_token_id == target_token_id:
                        predict_cpu[last_accepted_idx] = int(target_token_id)
                        accepted.append(draft_idx)
                        last_accepted_idx = draft_idx
                        break
                    cur_local_idx = next_sibling_cpu[offset + cur_local_idx]

                if cur_local_idx == -1:
                    break

            predict_cpu[last_accepted_idx] = int(target_predict_cpu[last_accepted_idx])
            accept_index_by_req.append(accepted)
            offset += draft_len

        self.predict = torch.tensor(predict_cpu, dtype=torch.int32, device=self.device)
        self.accept_index_by_req = accept_index_by_req
        self.accept_length = torch.tensor(
            [len(indices) - 1 for indices in accept_index_by_req],
            dtype=torch.int32,
            device=self.device,
        )
    ...
    def verify(
        ...
        bs = len(self.draft_lens_cpu) if self.has_variable_draft_lens else self.retrive_index.shape[0]
        ...
        if sampling_info.penalizer_orchestrator.is_required:
            ...
            if self.has_variable_draft_lens:
                repeat_counts = self.draft_lens.to(
                    device=linear_penalty.device, dtype=torch.int64
                )
                logits_bias = torch.repeat_interleave(
                    linear_penalty, repeat_counts, dim=0
                )
            else:
                logits_bias = torch.repeat_interleave(
                    linear_penalty, self.draft_token_num, dim=0
                )
            logits_output.next_token_logits.add_(logits_bias)
        ...
        if self.has_variable_draft_lens and not is_all_greedy:
            logger.warning(
                "TriePilot variable NGRAM budgets currently use greedy target-only "
                "verification; sampling verification is future work."
            )

        if is_all_greedy or self.has_variable_draft_lens or not TREE_SPEC_KERNEL_AVAILABLE:
