...
FLEXKV_VERBOSE_LOG = os.environ.get("FLEXKV_VERBOSE_LOG", "0") == "1"
FLEXKV_PROFILE = os.environ.get("FLEXKV_PROFILE", "0") == "1"
FLEXKV_PROFILE_PATH = os.environ.get(
    "FLEXKV_PROFILE_PATH",
    "/root/sglang_flex_test/my_experiments/results/flexkv_profile.jsonl",
)


def flexkv_profile_event(event: str, **fields) -> None:
    if not FLEXKV_PROFILE:
        return
    try:
        fields.update({
            "event": event,
        ...
        os.makedirs(os.path.dirname(FLEXKV_PROFILE_PATH), exist_ok=True)
        with open(FLEXKV_PROFILE_PATH, "a") as f:
            f.write(json.dumps(fields, ensure_ascii=False, default=str) + "\n")
    except Exception as e:
        logger.debug(f"[FlexKV Profile] failed to write {event}: {e}")
...
class Req:
    ...
    def __init__(
        ...
        flexkv_blocks: Optional[List[str]] = None,
        flexkv_warmup_doc_id: Optional[str] = None,
        flexkv_top_k: int = 0,
        flexkv_doc_start: Optional[int] = None,
        flexkv_doc_split_points: Optional[List[int]] = None,
        flexkv_doc_order: Optional[List[int]] = None,
        flexkv_ordered_doc_split_points: Optional[List[int]] = None,
        flexkv_sp_block_id: Optional[str] = None,
        flexkv_sp_block_cached: bool = False,
        flexkv_auto_register: Optional[List[Dict]] = None,
    ):
        ...
        # FlexKV
        self.flexkv_blocks = flexkv_blocks
        self.flexkv_warmup_doc_id = flexkv_warmup_doc_id
        self.flexkv_top_k = flexkv_top_k
        self.flexkv_doc_start = flexkv_doc_start
        self.flexkv_doc_split_points = flexkv_doc_split_points
        self.flexkv_doc_order = flexkv_doc_order
        self.flexkv_ordered_doc_split_points = flexkv_ordered_doc_split_points
        self.flexkv_sp_block_id = flexkv_sp_block_id
        self.flexkv_sp_block_cached = flexkv_sp_block_cached
        self.flexkv_auto_register = flexkv_auto_register
        self.flexkv_parallel = getattr(sampling_params, 'flexkv_parallel', False)
        self.flexkv_sys_len = getattr(sampling_params, 'flexkv_sys_len', 10)

        # FlexKV CoW: indices allocated for this request that need to be freed manually
        self.flexkv_cow_indices: Optional[torch.Tensor] = None
        # FlexKV direct-reuse slots are reserved during the request but are
        # owned by BlockKV, so they must only be unreserved, not freed.
        self.flexkv_reserved_indices: Optional[torch.Tensor] = None
        # FlexKV: Block-independent attention mask for multi-document
        self.flexkv_block_mask: Optional[torch.Tensor] = None
        self.flexkv_block_mask_info: Optional[Any] = None
        # FlexKV: warmup 后注册的实际 block IDs (kv_{hash} 格式)
        self.flexkv_registered_block_ids: Optional[List[str]] = None
        # FlexKV: Document split points for auto-cache
        # (flexkv_auto_register is set from constructor parameter at line 554)

        # Retraction and caching state (used by reset_for_retract, init_next_round_input, etc.)
        ...
        self.flexkv_hit = False
        self.flexkv_cached_len = 0
        ...
        # Speculative decoding request-level counters. NGRAM verification updates
        # these after each target-verify round.
        self.spec_verify_ct = 0
        self.spec_accepted_tokens = 0

        # FlexKV slots retention
        self.flexkv_slots_retained = False

    def release_cow_slots(self, allocator=None):
        """Release FlexKV CoW slots."""
        if self.flexkv_reserved_indices is not None and allocator is not None:
            try:
                if hasattr(allocator, "release_reserved_indices"):
                    allocator.release_reserved_indices(self.flexkv_reserved_indices)
                elif hasattr(allocator, "release_reserved_pages"):
                    page_size = getattr(allocator, "page_size", 1)
                    page_ids = torch.unique(
                        torch.div(
                            self.flexkv_reserved_indices,
                            page_size,
                            rounding_mode="floor",
                        )
                    )
                    allocator.release_reserved_pages(page_ids)
            except Exception:
                pass
            self.flexkv_reserved_indices = None

        if self.flexkv_cow_indices is not None and allocator is not None:
            try:
                # FlexKV CoW slots are explicitly reserved during KV reuse.
                # Release the reservation first; otherwise allocator.free()
                # filters them out and the slots leak permanently.
                if hasattr(allocator, "release_reserved_indices"):
                    allocator.release_reserved_indices(self.flexkv_cow_indices)
                elif hasattr(allocator, "release_reserved_pages"):
                    page_size = getattr(allocator, "page_size", 1)
                    page_ids = torch.unique(
                        torch.div(
                            self.flexkv_cow_indices,
                            page_size,
                            rounding_mode="floor",
                        )
                    )
                    allocator.release_reserved_pages(page_ids)
                allocator.free(self.flexkv_cow_indices)
                # logger.debug(f"[BlockKV] Freed {len(self.flexkv_cow_indices)} CoW slots for req {self.rid}")
            except Exception as e:
                # logger.warning(f"[BlockKV] Failed to free CoW slots: {e}")
                pass
            self.flexkv_cow_indices = None
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def prepare_for_extend(self):
        ...
        # [FlexKV Safety] Fix prefix_indices > fill_ids BEFORE alloc_for_extend.
        # This can happen when KV block page-alignment adds empty slots not counted
        # in token_ids, causing prefix_indices to exceed fill_ids length.
        #
        # IMPORTANT: For FlexKV KV reuse (flexkv_hit=True), prefix_indices > fill_ids
        # is EXPECTED because cached blocks' tokens are NOT in fill_ids (only KV reused).
        # We should NOT trim in this case!
        for req in reqs:
            # Skip trimming for FlexKV KV reuse requests
            flexkv_hit = getattr(req, 'flexkv_hit', False)
            if flexkv_hit:
                # FlexKV KV reuse: prefix_indices contains cached KV slots
                # fill_ids contains only fresh + query tokens
                # prefix_indices > fill_ids is expected and correct, but the
                # cached token count should remain the real token count. CoW
                # may allocate page-aligned padding slots; those are retained
                # in flexkv_cow_indices/flexkv_reserved_indices for cleanup,
                # but must not inflate seq_lens/prefix_lens.
                cached_tokens = getattr(req, 'flexkv_cached_tokens', None)
                prefix_slots = len(req.prefix_indices)
                if cached_tokens is None:
                    req.flexkv_cached_tokens = prefix_slots
                elif cached_tokens != prefix_slots:
                    if 0 < cached_tokens < prefix_slots:
                        if FLEXKV_VERBOSE_LOG:
                            logger.info(
                                f"[FlexKV] cached token/slot mismatch before extend: Rid={req.rid}, "
                                f"cached_tokens={cached_tokens}, prefix_slots={prefix_slots}. "
                                f"Using cached_tokens and trimming prefix view; padding remains reserved."
                            )
                        req.prefix_indices = req.prefix_indices[:cached_tokens]
                        prefix_slots = cached_tokens
                    else:
                        logger.warning(
                            f"[FlexKV] invalid cached token/slot mismatch before extend: Rid={req.rid}, "
                            f"cached_tokens={cached_tokens}, prefix_slots={prefix_slots}. "
                            f"Falling back to prefix_slots."
                        )
                        req.flexkv_cached_tokens = prefix_slots
                req.extend_input_len = len(req.fill_ids)
                if FLEXKV_VERBOSE_LOG:
                    logger.info(
                        f"[FlexKV] Skipping trim for KV reuse: Rid={req.rid}, "
                        f"prefix={prefix_slots}, fill_ids={len(req.fill_ids)}"
                    )
                continue

            raw_extend = len(req.fill_ids) - len(req.prefix_indices)
            if raw_extend < 1:
                trim_to = len(req.fill_ids) - 1
                logger.warning(
                    f"[FlexKV] pre-alloc trim: Rid={req.rid}, "
                    f"prefix_indices ({len(req.prefix_indices)}) > fill_ids ({len(req.fill_ids)}). "
                    f"Trimming prefix_indices to {trim_to}."
                )
                req.prefix_indices = req.prefix_indices[:trim_to]
                req.extend_input_len = 1

        # FlexKV: For KV reuse requests, use extend_input_len for input_ids extraction
        # When prefix_indices > fill_ids, fill_ids[len(prefix_indices):] returns empty
        # Correct: input_ids = fill_ids (for FlexKV) or fill_ids[prefix_len:] (for normal)
        input_ids = []
        for r in reqs:
            flexkv_hit = getattr(r, 'flexkv_hit', False)
            if flexkv_hit:
                # FlexKV KV reuse: entire fill_ids is the "extend" part
                # cached blocks' KV is already in prefix_indices (not in fill_ids)
                input_ids.append(r.fill_ids)
            else:
                # Normal case: extend = fill_ids - prefix
                input_ids.append(r.fill_ids[len(r.prefix_indices):])

        flexkv_positions = []
        has_flexkv_custom_positions = False
        for r, ids in zip(reqs, input_ids):
            spans = getattr(r, "flexkv_extend_spans", None)
            if spans:
                req_positions = []
                for logical_start, logical_end, compact_start, compact_end in spans:
                    span_len = int(compact_end) - int(compact_start)
                    if span_len != int(logical_end) - int(logical_start):
                        req_positions = []
                        break
                    req_positions.extend(range(int(logical_start), int(logical_end)))
                if len(req_positions) == len(ids):
                    flexkv_positions.extend(req_positions)
                    has_flexkv_custom_positions = True
                    continue
            pre_len = (
                getattr(r, "flexkv_cached_tokens", 0)
                if getattr(r, "flexkv_hit", False)
                else len(r.prefix_indices)
            )
            flexkv_positions.extend(range(pre_len, pre_len + len(ids)))

        extend_num_tokens = sum(len(ids) for ids in input_ids)
        # seq_lens should represent total sequence length (prefix KV + extend tokens)
        # For FlexKV KV reuse: seq_len = prefix_len + extend_len (fill_ids length)
        # This matches FlashInfer's expectation: seq_len = pre_len + extend_input_len
        seq_lens = []
        for r in reqs:
            flexkv_hit = getattr(r, 'flexkv_hit', False)
            if flexkv_hit:
                # FlexKV KV reuse: seq_len = cached_tokens + fill_ids
                # cached_tokens is actual token count (from concat_kv.total_tokens)
                # NOT len(prefix_indices) which has page-alignment padding
                cached_tokens = getattr(r, 'flexkv_cached_tokens', 0)
                seq_lens.append(cached_tokens + len(r.fill_ids))
            else:
                seq_lens.append(len(r.fill_ids))

        orig_seq_lens = [
            seq_lens[i]
            if getattr(r, "flexkv_hit", False)
            else max(len(r.fill_ids), len(r.origin_input_ids))
            for i, r in enumerate(reqs)
        ]
        # FlexKV Fix: prefix_lens should use actual cached token count, not slot count
        prefix_lens = []
        for r in reqs:
            flexkv_hit = getattr(r, 'flexkv_hit', False)
            if flexkv_hit:
                cached_tokens = getattr(r, 'flexkv_cached_tokens', 0)
                prefix_lens.append(cached_tokens)
            else:
        ...
        self.flexkv_positions = (
            torch.tensor(flexkv_positions, dtype=torch.int64).to(
                self.device, non_blocking=True
            )
            if has_flexkv_custom_positions
            else None
        )

        # Allocate memory
        profile_alloc_t0 = time.perf_counter()
        out_cache_loc, req_pool_indices_tensor, req_pool_indices = alloc_for_extend(
            self
        )
        profile_alloc_ms = (time.perf_counter() - profile_alloc_t0) * 1000.0
        if FLEXKV_PROFILE:
            flexkv_reqs = [r for r in reqs if getattr(r, "flexkv_blocks", None) is not None]
            if flexkv_reqs:
                flexkv_profile_event(
                    "prepare_for_extend",
                    batch_size=len(reqs),
                    flexkv_reqs=len(flexkv_reqs),
                    extend_num_tokens=extend_num_tokens,
                    max_seq_len=max(seq_lens) if seq_lens else 0,
                    max_prefix_len=max(prefix_lens) if prefix_lens else 0,
                    sum_prefix_lens=sum(prefix_lens),
                    alloc_ms=profile_alloc_ms,
                    total_pre_alloc_ms=(profile_alloc_t0 - profile_t0) * 1000.0,
                    total_ms=(time.perf_counter() - profile_t0) * 1000.0,
                )

        # Set fields
        profile_req_loop_t0 = time.perf_counter()
        input_embeds = []
        ...
        for i, (req, seq_len, pre_len) in enumerate(zip(reqs, seq_lens, prefix_lens)):
            ...
            # Sanity check: should always pass after the pre-alloc FlexKV trim above.
            assert seq_len - pre_len == req.extend_input_len, (
                f"[FlexKV] extend mismatch after trim: Rid={req.rid}, "
                f"Seq={seq_len}, Pre={pre_len}, Diff={seq_len-pre_len}, "
        ...
        profile_req_loop_ms = (time.perf_counter() - profile_req_loop_t0) * 1000.0

        profile_logprob_t0 = time.perf_counter()
        if self.return_logprob:
        ...
        profile_logprob_tensor_ms = (time.perf_counter() - profile_logprob_t0) * 1000.0

        profile_field_assign_t0 = time.perf_counter()
        ...
        profile_field_assign_ms = (time.perf_counter() - profile_field_assign_t0) * 1000.0

        profile_mamba_t0 = time.perf_counter()
        if get_global_server_args().enable_mamba_extra_buffer():
        ...
        profile_mamba_ms = (time.perf_counter() - profile_mamba_t0) * 1000.0

        profile_encoder_t0 = time.perf_counter()
        if self.model_config.is_encoder_decoder:
            self.prepare_encoder_info_extend(input_ids, seq_lens)
        profile_encoder_ms = (time.perf_counter() - profile_encoder_t0) * 1000.0

        # Build sampling info
        profile_sampling_t0 = time.perf_counter()
        self.sampling_info = SamplingBatchInfo.from_schedule_batch(
            self,
            self.model_config.vocab_size,
        )
        profile_sampling_ms = (time.perf_counter() - profile_sampling_t0) * 1000.0
        if FLEXKV_PROFILE:
            flexkv_reqs = [
                r for r in reqs if getattr(r, "flexkv_blocks", None) is not None
            ]
            if flexkv_reqs:
                flexkv_profile_event(
                    "prepare_for_extend_detail",
                    batch_size=len(reqs),
                    flexkv_reqs=len(flexkv_reqs),
                    extend_num_tokens=extend_num_tokens,
                    max_seq_len=max(seq_lens) if seq_lens else 0,
                    max_prefix_len=max(prefix_lens) if prefix_lens else 0,
                    pre_alloc_ms=(profile_alloc_t0 - profile_t0) * 1000.0,
                    alloc_ms=profile_alloc_ms,
                    req_loop_ms=profile_req_loop_ms,
                    logprob_tensor_ms=profile_logprob_tensor_ms,
                    field_assign_ms=profile_field_assign_ms,
                    mamba_ms=profile_mamba_ms,
                    encoder_ms=profile_encoder_ms,
                    sampling_info_ms=profile_sampling_ms,
                    total_ms=(time.perf_counter() - profile_t0) * 1000.0,
                )
    ...
    def release_req(self, idx: int, remaing_req_count: int, server_args: ServerArgs):
        ...
        if hasattr(req, "release_cow_slots"):
            req.release_cow_slots(self.token_to_kv_pool_allocator)
    ...
    def prepare_for_decode(self):
        ...
        # FlexKV two-stage positions are only valid for the prefill/extend
        # compact token stream. Decode batches must use SGLang's native
        # one-token positions, otherwise cuda-graph replay sees a stale
        # prefill-length position tensor.
        self.flexkv_positions = None
        bs = len(self.reqs)
        ...
        if keep_indices is None:
            ...
            for i, req in enumerate(self.reqs):
                if not req.finished() and req not in chunked_req_to_exclude:
                    ...
                    # Release FlexKV CoW slots for finished requests
                    if hasattr(req, "release_cow_slots"):
                        req.release_cow_slots(self.token_to_kv_pool_allocator)
    ...
    def get_model_worker_batch(
        ...
        flexkv_positions = None
        flexkv_extend_spans = None
        if not self.forward_mode.is_decode_or_idle():
            flexkv_positions = getattr(self, "flexkv_positions", None)
            flexkv_extend_spans = [
                getattr(req, "flexkv_extend_spans", None) for req in self.reqs
            ]
        ...
        return ModelWorkerBatch(
            ...
            flexkv_copy_info=[getattr(req, "flexkv_copy_info", None) for req in self.reqs],
            flexkv_block_masks=[getattr(req, "flexkv_block_mask", None) for req in self.reqs],
            flexkv_mask_infos=[getattr(req, "flexkv_block_mask_info", None) for req in self.reqs],
            flexkv_positions=flexkv_positions,
            flexkv_extend_spans=flexkv_extend_spans,
        )
...
class ModelWorkerBatch:
    ...
    # FlexKV Deferred Copy Plan
    flexkv_copy_info: Optional[List[List[dict]]] = None
    flexkv_block_masks: Optional[List[torch.Tensor]] = None
    flexkv_mask_infos: Optional[List[Any]] = None
    flexkv_positions: Optional[torch.Tensor] = None
    flexkv_extend_spans: Optional[List[Any]] = None
