...
RUN_ID_PREFIX="${TRIEPILOT_STEP1_RUN_ID_PREFIX:-20260512_step1_static_ngram_tiers}"
...
MATCH_TYPES="${TRIEPILOT_STEP1_MATCH_TYPES:-BFS}"
...
for match_type in ${MATCH_TYPES}; do
  match_label="$(echo "${match_type}" | tr '[:upper:]' '[:lower:]')"
  for shape_spec in ${SHAPES}; do
    IFS=: read -r shape_name match_window bfs_breadth branch_length <<<"${shape_spec}"
    for dataset in ${DATASETS}; do
      run_id="${RUN_ID_PREFIX}_${dataset}_${shape_name}_${match_label}_seed${SEED}"
...
notes = [
    ...
    "",
    "| dataset | match type | shape | best APV budget | best mean TPOT budget | best p99 TPOT budget |",
    "| --- | --- | --- | ---: | ---: | ---: |",
]
for key in keys:
    subset = [
    ...
    dataset, match_type, mw, bfs = key
    notes.append(
        f"| {dataset} | {match_type} | mw{mw}/b{bfs} | {best_apv['budget']} | {best_tpot['budget']} | {best_p99['budget']} |"
    )
